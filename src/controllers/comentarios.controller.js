...
export const createComentario = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error interno del servidor', error: error.message }); // errores internos
  }
};
...
export const getComentarios = async (req, res) => {
    try {
      ...
      console.error('Error al obtener comentarios:', error); // Mejorar log para debugging
      res.status(500).json({ message: 'Error interno del servidor' }); // para errores internos
    }
  };
